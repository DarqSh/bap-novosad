# taken from https://github.com/redoxcode/micropython-stepper/blob/main/src/stepper/__init__.py?utm_source=chatgpt.com
import machine
import math
import time
    ...
    def step(self,d):
        if d>0:
            ...
            if self.enabled:
                ...
                self.step_value_func(0)
            self.pos-=1
        self.steps_done = self.steps_done + 1 # mine

    def step_n_times(self, d, n): # my function, IS BLOCKING AND IT IS OK
        for _ in range(n):
            self.step(d)
            time.sleep_ms(10) # figure
