...
An effective angular step of $0.1\,\degree$ was selected as the base output step angle of the platform, since it lets all angles featured in \cref{tab:best_combinations} be represented as integer multiples of this value, meaning a bigger step will be artificially achieved by stepping a couple of times with a smaller step.
That means that a reduction ratio 1:18 has to be achieved. 
DRV8825 stepper motor driver\footnote{\url{https://www.ti.com/lit/ds/symlink/drv8825.pdf}} which is used in this project allows for a reduction ratio up to 1:32 to be achieved via microstepping. A reduction ratio of 1:2 was chosen here, since higher microstepping ratios come with a reduced accuracy of the output \cite[sec.~Texas Instruments DRV8825]{by_HowAccurateMicrostepping_2016}.
That leaves 1:9 reduction ratio to be obtained using a gearbox.
...
\begin{tabular}{p{0.45\linewidth} p{0.45\linewidth}}
    ...
    \begin{itemize}[label=\textcolor{red!70!black}{\textbf{--}}]
        ...
        \item High component count due to the large number of rollers required for high reduction ratios. In our case, a cycloidal gearbox with a 1:9 reduction ratio would require 10 ball bearings for each of the two cycloidal disks, making the design unnecessarily complex for an amateur project.
    \end{itemize}
\end{tabular}
...
All things considered, a planetary gearbox is the best solution for us, featuring both symmetric arrangement of components and simplicity while still providing favourable mechanical characteristics.

\subsection{Choosing Teeth Count}
Unlike with a cycloidal gearbox, it is harder to achieve 1:9 reduction ratio only with one stage while still keeping the overall design compact. Thus, a two-stage design is needed.
...
In our case, a ring gear whill be part of the outer shell of the platform, thus it will be stationary. In our case, the following transmission ratio will be true:
\begin{equation}
    i = 1+\frac{Z_{\text{ring}}}{Z_{\text{sun}}}.
    \label{eq:transmission ratio}
\end{equation}
Considering the constraints given in the \cref{eq:geometric_constraint,eq:equal_spacing_constraint}, for a transmission ratio of 1:3 for one stage, the following teeth counts were chosen:
\begin{equation}
