...
A video showing the scanning procedure can be found \href{https://youtube.com/shorts/UBePfZQdfIo?feature=share}{here}.
A screen recording of the Open3D visualisation of the point cloud being built in real time can be found \href{https://youtu.be/SptaiVY_dwU}{here}.
...
After filtering, misalignments betwen individual scan slices became visible. 
...
\begin{listing}[H]
\caption{Code snippet with the possible misalignemnt compensation.}
...
\begin{listing}[H]
\caption{Code snippet containing two corrections.\protect\footnotemark[7]}
\begin{minted}[linenos, frame=single, breaklines]{python}
def lidarTo3D(distance_mm, scan_angle_deg, azimuth_deg):
    r = distance_mm/1000. # mm -> m
...
\end{minted}
\end{listing}
\footnotetext[7]{To be mathematically correct, both $x$ and $y$ should also be multiplied by \texttt{np.cos(np.deg2rad(1.5))}. However, for such a small angle, the cosine is close to one and the effect of this correction on $x$ and $y$ would therefore have been negligible.}
