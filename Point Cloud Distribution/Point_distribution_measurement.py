...
# DISTRIBUTION MEASUREMENT GLOBAL VARIABLES
Distribution_values = []
distribution_angle = 270
distribution_angle_tolerance = 0.1 # twosided!!! Also step size in angle between measurements is 0.7, so don't go over 0.35 for angle tolerance
# END
...
def parse_lidar_data(data):
    ...
    global distribution_angle_tolerance
    if start_angle <= distribution_angle and distribution_angle <= stop_angle:
        for i in range(0,len(angle)):
            if (distribution_angle-angle[i])<distribution_angle_tolerance:
                Distribution_values.append(distance[i])
                break
...
if __name__ == "__main__":
    ...
    while running:
        ...
        # bytes we have already read).
            ...
            x_line = [0, np.sin(np.radians(distribution_angle))*PLOT_MAX_RANGE]
            y_line = [0, np.cos(np.radians(distribution_angle))*PLOT_MAX_RANGE]
